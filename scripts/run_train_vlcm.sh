...
if [ "$DATA_SOURCE" = "huggingface" ]; then
    ...
    # OFFROAD Dataset
    OFFROAD_DATA_DIR="/scratch/aparame/Research/VLCM_Data_Collection/OFFROAD/Varied_Traversability_new"
...
if [ -n "$SCRATCH" ]; then
    ...
    echo "WARNING: using $REPO_ROOT for scratch space. This may fail if quota is low."
fi
...
apptainer exec --nv -B "$REPO_ROOT:$REPO_ROOT" -B "$BASE_SCRATCH:$BASE_SCRATCH" \
  ...
  "$CONTAINER_PATH" accelerate launch --num_processes 2 -m ma_vlcm.train \
