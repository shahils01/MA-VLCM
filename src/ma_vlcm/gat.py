...
class GNN_Model(MessagePassing):
    ...
    def setup_layers(self):
        ...
        self.atts = nn.ParameterList(
            [
                nn.Parameter(torch.empty(self.heads, 2 * self.hid_channels))
    ...
    def aero_propagate(self, h, edge_index):
        ...
        for k in range(self.K):
            ...
            h_flat = h.reshape(
                -1, self.heads, self.hid_channels
            ...
            # Aggregate messages
            out = torch.zeros(
                h_flat.size(), dtype=messages.dtype, device=messages.device
            )
    ...
    def edge_att_pred(self, z_scale_i, z_scale_j, edge_index_batch, hop_k):
        ...
        if torch.isnan(a_ij).any():
            raise ValueError(
                "NaNs detected in edge attention normalization while handling layer k+1 = {self.k}"
            )
