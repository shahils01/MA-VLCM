...
from .sampling import balanced_tb3_shards, split_tb3_shards_by_stratum
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default="mixed",
        help="Type of dataset (e.g., rware, offroad, mixed, tb3_progress_v2)",
    )
    p.add_argument("--success_only", action="store_true")
    p.add_argument("--balance_tb3_sources", action="store_true")
    p.add_argument(
        "--tb3_balance_mode",
        type=str,
        default="domain_cardinality",
        choices=["domain", "domain_cardinality"],
        help=(
            "Balance TB3 shards by task domain alone or by the full "
            "domain/cardinality cross-product."
        ),
    )
    p.add_argument("--target_schema", type=str, default="")
    p.add_argument(
        "--tb3_image_mode",
        type=str,
        ...
        help=(
            "Canonicalize TB3 overhead frames without rotation or reflection. "
            "center_square removes horizontal letterboxing from legacy 640x360 "
            "frames and preserves already-square Isaac frames."
        ),
    )
...
def _progress_target_from_sample(
    ...
    if str(target_schema).lower() == "tb3_progress_v2":
        ...
        initial = np.asarray(initial_goal_distances, dtype=np.float32)
        if initial.shape != current.shape:
            raise ValueError(
                "tb3_progress_v2 initial/current distance shape mismatch: "
                f"{initial.shape} versus {current.shape}"
            )
        denominator = np.maximum(initial, 1e-6)
        per_agent = np.clip((initial - current) / denominator, 0.0, 1.0)
        return float(per_agent.mean()) if per_agent.size else 0.0
...
def _parse_tb3_lab_state(
    ...
    V2 appends ``initial_goal_distance`` to the legacy eight features.
    """
    ...
    for i, ag in enumerate(agents):
        ...
        if isinstance(vel, dict):
            v_lin = float(vel.get("linear", 0.0))
            v_ang = float(vel.get("angular", 0.0))
        else:
            v_lin = float(vel[0]) if len(vel) > 0 else 0.0
            v_ang = float(vel[1]) if len(vel) > 1 else 0.0
        ...
        if robot_obs_dim >= 9:
            initial = (
                initial_goal_distances[i]
                if initial_goal_distances is not None and i < len(initial_goal_distances)
                else ag.get(
                    "initial_goal_distance",
                    ag.get("initial_dist_to_goal", ag.get("dist_to_goal", 0.0)),
                )
            )
            obs[i, 8] = float(initial)

    return obs


def _split_shard_sources(shards):
    """Split a semicolon-delimited set of dataset sources."""

    if not isinstance(shards, str):
        return shards
    sources = [source.strip() for source in shards.split(";") if source.strip()]
    return sources or [shards]


def _expand_single_hf_dataset_shards(shards):
    """Download and expand one hf://datasets/<owner>/<repo>/<glob> source."""
    if not isinstance(shards, str):
        return shards
    prefix = "hf://datasets/"
...
def _expand_hf_dataset_shards(shards):
    """Expand one or more semicolon-delimited shard sources.

    The resulting cache paths retain Hugging Face repository names, which the
    TB3 sampler uses to identify the legacy goal-to-goal and obstacle sources.
    """

    sources = _split_shard_sources(shards)
    if not isinstance(sources, list) or len(sources) == 1:
        source = sources[0] if isinstance(sources, list) else sources
        return _expand_single_hf_dataset_shards(source)

    expanded = []
    for source in sources:
        resolved = _expand_single_hf_dataset_shards(source)
        if isinstance(resolved, list):
            expanded.extend(resolved)
        elif isinstance(resolved, str) and os.path.isdir(resolved):
            expanded.extend(
                sorted(glob.glob(os.path.join(resolved, "**", "*.tar"), recursive=True))
            )
        elif isinstance(resolved, str) and any(char in resolved for char in "*?["):
            expanded.extend(sorted(glob.glob(resolved, recursive=True)))
        else:
            expanded.append(resolved)
    print(f"Resolved {len(expanded)} total shards from {len(sources)} sources.")
    return expanded


def _canonicalize_tb3_image(image, mode, resize_width, resize_height):
    """Preserve TB3 world/image axes while standardizing camera geometry."""

    if mode == "center_square":
        if resize_width != resize_height:
            raise ValueError(
                "TB3 center_square canonicalization requires equal resize width "
                f"and height, got {resize_width}x{resize_height}."
            )
    ...
    if resize_width > 0 and resize_height > 0 and image.size != (
        resize_width,
        resize_height,
    ):
...
def _tb3_task_context(state_json, sample=None):
    """Return a canonical domain-specific instruction and prompt metadata."""

    agents = state_json.get("agents", [])
    num_agents = int(state_json.get("num_agents", len(agents)))
    task_name = str(state_json.get("task", "")).strip()
    stored_instruction = str(state_json.get("task_instruction", "")).strip()
    if not stored_instruction and sample is not None:
        task_value = sample.get("task.txt")
        if isinstance(task_value, bytes):
            stored_instruction = task_value.decode("utf-8", errors="replace").strip()
        elif isinstance(task_value, str):
            stored_instruction = task_value.strip()

    domain = str(state_json.get("task_domain", "")).strip().lower().replace("-", "_")
    evidence = f"{task_name} {stored_instruction}".lower().replace("-", "_")
    if not domain:
        if "static obstacle" in evidence or re.search(r"marl(?:_hpc)?_v(?:1|3(?:\.0)?)", evidence):
            domain = "static_obstacles"
        else:
            domain = "goal_to_goal"

    if domain == "static_obstacles":
        num_obstacles = int(state_json.get("num_static_obstacles", 3))
        instruction = (
            f"Coordinate exactly {num_agents} TurtleBot3 agents so every agent reaches "
            f"its assigned goal while avoiding all {num_obstacles} static obstacles "
            "and collisions with teammates."
        )
    else:
        num_obstacles = 0
        instruction = (
            f"Coordinate exactly {num_agents} TurtleBot3 agents so every agent reaches "
            "its assigned goal while avoiding collisions with teammates. This "
            "goal-to-goal environment has no static obstacles."
        )
...
def _build_tb3_prompt(
    ...
    environment = str(state_json.get("environment", "simulation")).lower()
    environment_label = (
        "physical indoor"
        if environment in {"physical", "real", "lab_hardware"}
    ...
    header = (
        ...
        f"and exactly {num_obstacles} static obstacles. "
        "The overhead image and structured positions use one aligned world frame in meters: "
...
class SequenceWebDataset(IterableDataset):
    def __init__(
        ...
        if balance_tb3_sources:
            if not isinstance(shards, list):
                raise ValueError("balanced TB3 sampling requires expanded local shard paths")
            counts = [value for value in str(trained_agent_counts).split(",") if value]
            domains = [value for value in str(task_domains).split(",") if value]
            shards = balanced_tb3_shards(
                shards, counts, domains, balance_mode=tb3_balance_mode
            )
            group_count = (
                len(domains)
                if tb3_balance_mode == "domain"
                else len(counts) * len(domains)
            )
            print(
                f"Balanced TB3 sampling across {group_count} "
                f"{tb3_balance_mode} groups."
            )
            ...
            if "overhead" in key or "image" in key or "rware_topdown" in key:
                try:
                    with io.BytesIO(data) as stream:
                        ...
                        if "rware_topdown" not in key and not self.dataset_type.startswith("tb3"):
    ...
    def __iter__(self):
        ...
        for sample in dataset:
            ...
            iw, ih = image.size
            if self.dataset_type.startswith("tb3"):
                image = _canonicalize_tb3_image(
                    image,
                    self.tb3_image_mode,
                    self.resize_width,
                    self.resize_height,
                )
            elif self.rware_visual_mode == "both" and self.dataset_type == "rware":
            ...
            if self.dataset_type == "rware":
            ...
            elif self.dataset_type.startswith("tb3"):
                # -------------- REAL LAB TURTLEBOT3 HANDLING --------------
                try:
                    state_json = sample["state.json"]
                    if isinstance(state_json, bytes):
                        state_json = json.loads(state_json)
                    elif isinstance(state_json, str):
                        state_json = json.loads(state_json)
                except Exception as e:
                    print(f"Error parsing tb3_lab state.json: {e}")
                    continue

                progress_metadata = _json_from_sample(sample, "progress.json") or {}
                has_episode_success = "episode_success" in progress_metadata
                success = (
                    bool(progress_metadata.get("episode_success"))
                ...
                if self.success_only:
                    if success is None:
                        raise ValueError(
                            "--success_only requires episode_success labels. Relabel "
                            "legacy tb3_progress_v1 shards or disable SUCCESS_ONLY."
                        )
                    if not success:
                        continue
                agents = state_json.get("agents", [])
                ...
                if "reward.json" in sample:
                    reward = sample["reward.json"]
                    if isinstance(reward, bytes):
                        reward = json.loads(reward)
                    reward = float(reward)
                elif "reward" in state_json:
                    reward = float(state_json.get("reward", 0.0))
                else:
                    agents = state_json.get("agents", [])
                    reward = float(
                        np.mean([ag.get("reward", 0.0) for ag in agents]) if agents else 0.0
                    )
                progress = _progress_target_from_sample(
                    sample,
                    state_json,
                ...
                agents = state_json.get("agents", [])
                outcome = episode_meta.get("outcome")
                if outcome is None:
                    if episode_meta.get("failure", False):
                        outcome = "failure"
                    elif episode_meta.get("success", False):
                        outcome = "success"
                    elif episode_meta.get("done", False):
                        outcome = "done"
                    else:
                        outcome = "running"
                termination_reason = episode_meta.get("termination_reason", "")
                done = bool(
                    episode_meta.get(
                        "done",
                        all([ag.get("reached", False) for ag in agents]) if agents else False,
                    )
                )
                ...
                for ag in visible_agents:
                    ...
                    if isinstance(vel, dict):
                        ...
                        v_lin = float(vel[0]) if len(vel) > 0 else 0.0
                        v_ang = float(vel[1]) if len(vel) > 1 else 0.0
                    goal_label = ag.get("goal_label", "?")
                    goal_pos = ag.get("goal_pos", [0.0, 0.0])
                    dist_to_goal = ag.get("dist_to_goal", 0.0)
                    min_neighbor_dist = ag.get("min_neighbor_dist", 0.0)
                    reached = "yes" if ag.get("reached", False) else "no"
                    collision = "yes" if ag.get("collision", False) else "no"

                    obs_lines.append(
                        f"Agent {ag_id} ({color}): position ({pos[0]:.2f}, {pos[1]:.2f}), "
                        f"heading {yaw:.2f} rad, linear_vel {v_lin:.2f} m/s, angular_vel {v_ang:.2f} rad/s, "
                        f"goal {goal_label} at ({goal_pos[0]:.2f}, {goal_pos[1]:.2f}), "
                        f"dist_to_goal {dist_to_goal:.2f}m, min_neighbor_dist {min_neighbor_dist:.2f}m, "
                        f"reached: {reached}, collision: {collision}."
                    )

                step_idx = episode_meta.get("step", episode_frame_count)
                if self.text_prompt_template is None:
                    text = _build_tb3_prompt(
                        state_json,
                        sample,
                        obs_lines,
                        step_idx,
...
def webdataset_loader(
    ...
    dynamic_team_size = str(dataset_type).startswith("tb3")
    dataset = SequenceWebDataset(
        ...
        success_only=getattr(args, "success_only", False),
        target_schema=getattr(args, "target_schema", ""),
        tb3_image_mode=getattr(args, "tb3_image_mode", "none"),
        balance_tb3_sources=(
            getattr(args, "balance_tb3_sources", False)
            if balance_tb3_sources is None
            else bool(balance_tb3_sources)
        ),
    ...
    for batch in pbar:
        ...
        with accelerator.accumulate(model):
            with torch.set_grad_enabled(train):
                return_features = (
                    args.loss_type in ("contrastive", "contrastive_mse")
                    and getattr(args, "contrastive_multidepth", False)
                ) or float(getattr(args, "success_loss_weight", 0.0)) > 0.0
                model_out = model(
                ...
                if success_weight > 0.0:
                    ...
                    success_logit = model_out.get("success_logit") if isinstance(model_out, dict) else None
                    if success_target is None or success_logit is None:
                        raise RuntimeError("success-head training requires success labels and return_features")
                    success_loss = F.binary_cross_entropy_with_logits(
                        success_logit.view(-1),
                        success_target.to(accelerator.device).view(-1),
                    )
...
def split_shards(shards_pattern, val_split=0.2, seed=42, stratify_tb3=None):
    import random
    ...
    if stratify_tb3 is not None:
        agent_counts, task_domains, balance_mode = stratify_tb3
        return split_tb3_shards_by_stratum(
            files,
            agent_counts,
            task_domains,
            val_split=val_split,
            seed=seed,
            require_all=balance_mode == "domain_cardinality",
        )
...
def main():
    ...
    if args.val_shards:
        ...
        stratify_tb3 = None
        if args.balance_tb3_sources and str(args.dataset_type).startswith("tb3"):
            stratify_tb3 = (
                [value for value in args.trained_agent_counts.split(",") if value],
                [value for value in args.task_domains.split(",") if value],
                args.tb3_balance_mode,
            )
        train_main_shards, val_main_shards = split_shards(
            args.train_shards, args.val_split, args.split_seed,
            stratify_tb3=stratify_tb3,
        )
    ...
    if str(args.dataset_type).startswith("tb3"):
        accelerator.print(
            f"Single-dataset training enabled for {args.dataset_type} TurtleBot data."
        )
