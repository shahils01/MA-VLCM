...
# From MA-VLCM/src/ma_vlcm/train.py
SHELF_MAP = {
    1: (1, 1),
    2: (2, 1),
    3: (7, 1),
...
def calculate_step_reward(state_data, dist_matrix, times_col):
    """
    Replicates the reward logic from MA-VLCM/src/ma_vlcm/train.py
    """
    # 1. Base Reward
    base_reward = 0.0
...
def process_config(config_name, shards, args):
    ...
    out_dir = "outputs/plots/reward_analysis"
    os.makedirs(out_dir, exist_ok=True)
    out_file = os.path.join(out_dir, f"analysis_{safe_name}.png")
    ...
    if tars_direct:
        ...
        if configs_found:
            ...
            if any("step" in f.name for f in input_path.iterdir()):
                ...
                out_dir = "outputs/plots/reward_analysis"
                os.makedirs(out_dir, exist_ok=True)
                out_file = os.path.join(out_dir, "analysis_single_trajectory.png")
                plt.savefig(out_file)
                print(f"Saved plots to {out_file}")
                plt.close()
            else:
